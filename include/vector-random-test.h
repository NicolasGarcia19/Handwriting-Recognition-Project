...
void test_case_find_closest_linear_random(int test_case_num, Func f,
                                          DistFunc dist) {
  std::printf("\n%d: %s\n", test_case_num, __func__);
  std::srand(0xdeadbeef);
  ...
  for (int i = 0; i < 10; i++) {
    ...
    for (int j = 0; j < size; j++)
      ECE2400_CHECK_TRUE(vec.find_closest_linear(f(j * offset + 1), dist) ==
                         f(j * offset));
...
void test_case_find_closest_binary_random(int test_case_num, Func f, int k,
  ...
  for (int i = 0; i < 10; i++) {
    ...
    // Check the closest value for the multiples of offset plus one
    for (int j = 0; j < size; j++)
      ECE2400_CHECK_TRUE(vec.find_closest_binary(f(j * offset + 1), k, dist,
                                                 cmp) == f(j * offset));
...
template <typename T, typename Func, typename DistFunc, typename CmpFunc>
void test_case_general_random(int test_case_num, Func f, int k, DistFunc dist,
                              CmpFunc cmp) {
  std::printf("\n%d: %s\n", test_case_num, __func__);
  std::srand(0xdeadbeef);
  ...
  for (int i = 0; i < 10; i++) {
    ...
    // Test find_closets for value beyond size
    ECE2400_CHECK_TRUE(vec.find_closest_binary(f(size), k, dist, cmp) ==
                       f(size - 1));
