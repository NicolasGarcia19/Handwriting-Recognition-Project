...
std::string ece2400::OutOfRange::to_str() const { return m_err_msg; }
...
std::string ece2400::InvalidArgument::to_str() const { return m_err_msg; }
...
void ece2400::sort(int *a, int size) { std::sort(a, a + size); }
...
void ece2400::timer_reset() { gettimeofday(&ece2400::start_time, NULL); }
...
void ece2400::__check_and_print_int_binop(const char *file, int lineno,
  ...
  if (ece2400::__failure_condition) {
    ...
    std::printf(" - [ " GREEN "passed" RESET " ] %s:%d:  %s == %s (%d == %d)\n",
                file, lineno, expr1, expr2, ece2400::__int_expr0,
                ece2400::__int_expr1);
  } else if (ece2400::__n < 0) {
    std::printf(GREEN "." RESET);
...
void ece2400::__check_and_print_char_binop(const char *file, int lineno,
  ...
  if (ece2400::__failure_condition) {
    ...
    std::printf(" - [ " GREEN "passed" RESET " ] %s:%d:  %s == %s (%c == %c)\n",
                file, lineno, expr1, expr2, ece2400::__char_expr0,
                ece2400::__char_expr1);
  } else if (ece2400::__n < 0) {
    std::printf(GREEN "." RESET);
